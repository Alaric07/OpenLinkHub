...
func darkstarWWorkspaceFixture() *Device {
	...
	return &Device{Serial: "darkstar-w", MinDPI: 100, MaxDPI: 26000, DPIAmount: 5, KeyAssignment: buttons, KeyAssignmentTypes: map[int]string{0: "None", 1: "Media"}, SleepModes: map[int]string{15: "15 minutes"}, LiftHeights: map[int]string{2: "Low"}, SwitchModes: map[int]string{0: "Disabled", 1: "Enabled"}, UserProfiles: map[string]*DeviceProfile{"Default": {Active: true}}, DeviceProfile: &DeviceProfile{Profile: 2, DPIColor: &color, Profiles: profiles, SleepMode: 15, ButtonOptimization: 1, LiftHeight: 2, Tilts: map[int]TiltOption{0: {Name: "Left", Value: 20}, 1: {Name: "Right", Value: 20}, 2: {Name: "Forward", Value: 20}, 3: {Name: "Back", Value: 20}}}}
}
...
func TestWorkspaceSnapshots(t *testing.T) {
	d := darkstarWWorkspaceFixture()
	if s, ok := d.DPISnapshot(); !ok || s.ActiveRegularStageID != "2" || len(s.Stages) != 6 {
		t.Fatal("DPI snapshot")
	}
	if s, ok := d.ButtonsSnapshot(); !ok || len(s.Buttons) != 19 || len(s.AssignmentTypes) != 2 {
		t.Fatal("buttons snapshot")
	}
	if s, ok := d.PerformanceSnapshot(); !ok || s.PollingRate != nil {
		t.Fatal("performance snapshot")
	}
	if s, ok := d.SleepTimerSnapshot(); !ok || s.Value != 15 {
		t.Fatal("sleep snapshot")
	}
	if s, ok := d.MouseGesturesSnapshot(); !ok || len(s.Tilts) != 4 || s.Tilts[0].ID != "0" {
